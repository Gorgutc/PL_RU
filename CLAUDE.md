...
- A13: workspace shell, left sidebar, side-panel alignment, and real MapLibre/OSM map contract stay frozen.
...
- A17: per-tab chrome responsive adaptation (compact overflow-to-dropdown below `1920`, `2560` content max-width) and the per-tab bottom panels (`TabBottomPanel`) stay on their frozen contract (see `docs/agent/frozen-decisions.md`).
