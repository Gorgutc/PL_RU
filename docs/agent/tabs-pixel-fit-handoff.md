...
- Статус: **I2 завершён — инструментарий референсов работает, диагностика
  снята** (см. журнал). Draft PR-A открыт:
  <https://github.com/Gorgutc/PL_RU/pull/21> (мерж за пользователем).
  Далее: I3 — zero-delta рефакторинг MapBottomPanel → TabBottomPanel + A17.
