...
- Статус: **I5 в работе — RoutesPanel собран и закоммичен (структура по
  рефам, baseline перезахвачен, e2e зелёные); идёт пиксельная полировка**.
  Текущий design:check bar bottom-panel ≈ 13.7–14.9% mismatch (допуск 6.5%):
  доминируют ширина карточки фильтров (зазоры свитчей у нас шире реф) и
  AA-шум; legend-юнит уже inline. Цикл полировки: замерить реф-границы
  карточек по кропам → подвести зазоры/паддинги → design:check, повторять.
  Draft PR-A: <https://github.com/Gorgutc/PL_RU/pull/21>, draft PR-B (WIP,
  стек): <https://github.com/Gorgutc/PL_RU/pull/22>.
  Далее: добить полировку bar (вкл. F1/F3/F6), финализировать I5
  (visual_qa_guardian + component_reuse_guardian), затем I6 (tmi).
...
- Натуральная ширина даты = 112px (`$top-controls-datetime-min-width`) —
  **единственное место, где значение зафиксировано числом**; если кроп 1280
  ...
  Замер кропов 1280 (visual-QA I4): реф-поля ≈96px (tmi ровно 96/96, bar
  97/103) — финальное значение калибруется в I5/I6 пиксельной подгонкой;
  до тех пор действует 112px.
