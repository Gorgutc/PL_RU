...
- Статус: **I5 полировка завершена — bar тулбар + нижняя панель пиксельно
  подогнаны по 8 рефам; геометрия карточек/контролов сидит ±3px; design:check
  bar: 25/32 PASS** (FAIL только общий chrome header/rail на 1280/1920 —
  реальные дельты типа зелёного индикатора «Данные», зона I7).
  Остаточный mismatch — шрифтовой пол (Figma кладёт Inter дробными
  advance, Chrome хинтует: глифы двоятся на 1–3px; tracking-эксперимент
  ухудшил — откатен). Закрыто per-case tolerance в `PER_CASE_TOLERANCE`
  (bar top-controls/bottom-panel 1920: 0.07–0.085, 1280: 0.095–0.105) —
  **требует подтверждения пользователя (stop rule 2)**. Капчуры
  нормализованы `--disable-lcd-text` (grayscale AA как у Figma-экспортов).
  Draft PR-A: <https://github.com/Gorgutc/PL_RU/pull/21>, draft PR-B (WIP,
  стек): <https://github.com/Gorgutc/PL_RU/pull/22>.
  CI-каскад, слой 4 (закрыт): оба прогона 2026-06-12 повисли на
  `playwright install` (Node 24.16+ регрессия extract-zip,
  microsoft/playwright#41000) — CI запинен на Node 24.15.0
  (PR-A `7872f2b`, PR-B черри-пик `d36bb66`), анпин при бампе
  Playwright ≥1.60 в I9.
  Далее: финализация I5 субагентами → коммит → I6 (tmi: панель + тулбар).
