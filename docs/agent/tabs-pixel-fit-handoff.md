...
- Статус: **I3 завершён — TabBottomPanel-диспетчер на месте, A17 расширен
  атомарно, zero-visual-delta доказан** (см. журнал). Draft PR-A открыт:
  <https://github.com/Gorgutc/PL_RU/pull/21> (мерж за пользователем).
  Далее: I4 — фикс переполнения compact-полосы (наезд «Тип данных») +
  даты без « | 00:00» на bar/tmi.
