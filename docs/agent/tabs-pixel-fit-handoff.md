...
- Статус: **I4 завершён — compact-полоса bar/map чиста, per-tab резина
  внедрена, даты date-only** (см. журнал; tmi-наезд — документированный
  interim до фолда селектов в I6). Draft PR-A открыт:
  <https://github.com/Gorgutc/PL_RU/pull/21> (мерж за пользователем).
  Далее: I5 — панель bar + пиксельная подгонка bar по 8 рефам
  (вкл. маршрутизированные находки F1/F3/F6 из I4).
