...
- Статус: **PR-A готов — I0+I1 завершены, codex:ship PASS** (полный гейт:
  build + e2e 51/51 + a11y + visual + lighthouse + refs 8/8). Ветка пушится,
  открывается draft PR-A (ссылка — в журнале следующего чекпойнта). Далее:
  I2 на ветке `claude/tabs-pixel-fit` от головы PR-A.
