...
- Статус: **Серия визуальных багфиксов №1 — ЗАВЕРШЕНО, ветка обновлена**. Багфиксы #1–#3 (растяжение карт + гибкая дата/время; центрирование иконок + зазор групп 16px; общий `SelectControl`) + усиление по Codex-ревью №3 (3 major / 2 minor, без блокеров). `verify` 22/22, e2e 33/33, `check:visual` PASS. Коммиты `5d33024`, `0284f1b`, `588437c`. Запушено в origin; draft PR — за пользователем.
